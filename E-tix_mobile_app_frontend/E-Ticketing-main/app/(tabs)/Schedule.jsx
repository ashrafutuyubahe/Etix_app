  ...
  function Schedule() {
    ...
    const fetchSeatData = async (carPlate) => {
      try {
        const response = await fetch(`http://192.168.43.76:2000/api/seats/getSeat?carPlate=${carPlate}`, {
          method: "GET",
        });
        ...
        if (seatData && seatData.availableSeats) {
          setSeats(seatData.availableSeats);
        } else {
          console.error("Invalid seat data format");
          return "Unknown";
        }
      } catch (error) {
    ...
    >
      ...
      <View style={{ marginTop: 15 }}>
        ...
        <Text style={styles.cardText}>Available Seats: {seats|| "Unknown"}</Text>
      </View>
      <View
          ...
          {schedules.map((schedule, index) => (
            <View key={index} style={styles.scheduleCard}>
              ...
              onPress={() => {
                ...
                setVisible(true);
                fetchSeatData(schedule.carPlate);
              }}
